      ...
      {tabs.map((tab) => (
        <div key={tab.id} className="col-span-1 h-full">
          <Link
            ...
            // className={`flex p-3 rounded-t-lg ${activeTab === tab.id ? 'bg-green-lighter relative before:content-["jj"] before:absolute before:-right-5 before:bottom-0 before:bg-green-lighter before:h-12 before:w-6' : 'text-white'}`}
          >
