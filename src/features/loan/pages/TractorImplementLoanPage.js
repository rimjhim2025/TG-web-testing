...
import EligibilityAndDocumentData from '@/src/data/loan-Eligibility-document-data/tractorImplementLoan.json';
...
export default async function TractorImplementLoanPage({ propsPrefLang }) {
  const prefLang = propsPrefLang === 'hi' ? 'hi' : await getSelectedLanguage();
  const page = 'tractor-implement-loan';
  ...
  return (
    <>
      ...
      <main className="lg:mt-[159px]">
        <LoanBanner
          title={translation?.loan?.implementLoan}
          page={page}
        ...
        <LoanImplementForm
          isMobile={isMobile}
          translation={translation}
          title={translation?.headings?.applyforImplementLoan}
        />
        <LoanEligibilityAndDocuments data={EligibilityAndDocumentData} currentLang={prefLang} />
        <LoanFinanceSchemes title={translation?.loan?.implementLoanFinanceSchemes} />

        {faqs?.length > 0 && (
          <TyreFAQs
            headingKey="loan.implementLoanFaq"
            translation={translation}
            faqs={faqs}
            faqsError={faqsError}
          />
