...
export default async function ElectricTractorsPage({ params, searchParams }) {
  const currentLang = await getSelectedLanguage(); // Server-side language detection
  const translation = await getDictionary(currentLang);
  const config = getTractorPageConfig('electricTractors', translation);
