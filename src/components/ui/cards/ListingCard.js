...
import { tgi_arrow_right, tgi_star, tgi_star_half_filled } from '@/src/utils/assets/icons';

const TG_HorizontalCard = ({
  title,
  imageSrc,
  imageAlt,
  detailUrl,
  ...
  position = null, // Position in list for SEO
  tractorId = null, // Tractor ID for SEO
}) => {
  return (
    <>
      {/* SEO Structured Data - Hidden from UI */}
      {position && tractorId && (
        <div
        ...
        >
          ...
          <meta itemProp="url" content={`https://tractorgyan.com${detailUrl}`} />
                ...
                {showRatingOnTop && (
                  <div className="flex flex-col items-end gap-0 font-medium text-gray-dark">
                    ...
                    <span className="text-nowrap text-xs">
                      {total_reviews > 0 ? `${total_reviews} Reviews` : 'No Reviews'}
                    </span>
                  </div>
                )}
              </div>
